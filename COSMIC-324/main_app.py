...
# ============================================================
# 📁 إعدادات ملف العقود وبيانات النظام
# ============================================================
...
# ============================================================
# 🌍 نظام الترجمة واتجاه الصفحة والدول العالمية الشاملة
# ============================================================
LANGUAGES = {
    "ar": {
        "name": "العربية",
...
# ============================================================
# 🌐 الشريط الجانبي وتحديد الأقسام الخمسة
# ============================================================
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("""
    <div style="background-color: #1a1a2e; padding: 10px; border-radius: 5px; text-align: center;">
        <span style="color: #00CCFF; font-weight: bold;">Stripe</span> &nbsp;|&nbsp; <span style="color: #0070BA; font-weight: bold;">PayPal</span>
        <p style="font-size: 11px; color: #aaa; margin-top: 5px;">بوابات الدفع السيادية المعتمدة</p>
    </div>
    """, unsafe_allow_html=True)

# ============================================================
# 📡 جلب البيانات وتسريع الحسابات المدارية (J2 Perturbations)
# ============================================================
...
if app_section == t('nav_dashboard'):
    ...
    col3.metric(t('configured_stations'), len(ALL_COUNTRIES))
    ...
    selected_country_name = st.selectbox(t('gs_select'), country_names, index=country_names.index("United States") if "United States" in country_names else 0)
    selected_country = next(c for c in ALL_COUNTRIES if c["name"] == selected_country_name)

    h = int(hashlib.md5(selected_country_name.encode('utf-8')).hexdigest(), 16)
    gs_lat = float((h % 160) - 80)
    gs_lon = float(((h // 160) % 360) - 180)

    st.info(f"📍 الإحداثيات النشطة للدولة ({selected_country_name}): **خط العرض ({gs_lat:.2f})** | **خط الطول ({gs_lon:.2f})**")
    ...
    if not df_visible.empty:
        ...
        st.warning("لا توجد أقمار صناعية حالياً ضمن نطاق الرؤية المباشرة لهذه الدولة.")
    ...
    def render_cosmic_globe(df, gs_lat, gs_lon, station_name, title="🌍 3D Constellation Globe"):
        ...
        short_station_label = f"🛰️ {station_name}"
        fig.add_trace(go.Scattergeo(
    ...
    st.plotly_chart(render_cosmic_globe(df, gs_lat, gs_lon, selected_country_name, t('3d_globe')), use_container_width=True)
...
# 2️⃣ إدارة التراخيص (Licenses Management)
    ...
    with col_l1:
        ...
        tier_input = st.selectbox(t('license_tier'), ["الباقة الأساسية ($49)", "الباقة السيادية Titan X ($199)", "باقة وكالات الفضاء (مخصص)"])
    with col_l2:
...
# 3️⃣ العملاء وبوابات الدفع (Clients Portal & Stripe/PayPal)
    ...
    with tab_pay:
        ...
        st.markdown("محاكاة بوابات الدفع العالمية الآمنة المعتمدة في النظام (**Stripe** و **PayPal**).")
        col_p1, col_p2 = st.columns([2, 1])
        with col_p1:
            st.markdown("""
            * **الباقة**: الباقة السيادية Titan X (الوصول الشامل لـ 5000 قمر صناعي ودعم عالمي)
            * **المبلغ المستحق**: **$199.00 USD**
            * **معرف المعاملة الآمنة**: `PAY-GLOBAL-COSMIC324-2026`
            """)
        with col_p2:
            gateway_choice = st.radio("اختر بوابة الدفع:", ["Stripe Checkout", "PayPal Express"])
            if st.button(t('pay_now')):
                st.success(f"✅ تم اتمام الدفع بنجاح عبر بوابة **{gateway_choice}** وتفعيل الاشتراك السيادي فوراً!")
                st.balloons()
...
st.markdown("""
<div class='copyright'>
    <p>🛰️ COSMIC-324: 6G Titan X Global Edition v8.0 - Stripe & PayPal Integrated</p>
